...
<div class="mb-5">
    ...
    <p class="mt-1 text-sm text-gray-500">Selecione o marketplace e o período para ver quais estabelecimentos tiveram venda no EDI (incluindo maquininha) e baixar o Excel.</p>
</div>
...
@if ($marketplace)
    ...
    @if ($resumo)
        <div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <p class="text-xs font-medium text-gray-500">Com transação</p>
                <p class="mt-1 text-xl font-bold tabular-nums text-emerald-600">{{ number_format($resumo['com_transacao'], 0, ',', '.') }}</p>
            </div>
            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <p class="text-xs font-medium text-gray-500">Sem transação</p>
        ...
        <p class="mb-3 text-xs text-gray-500">
            ...
            · Sem transação com token: {{ number_format($resumo['sem_transacao_com_token'], 0, ',', '.') }}
            · Terminal: {{ number_format($resumo['qtd_terminal'], 0, ',', '.') }} de {{ number_format($resumo['qtd_transacoes'], 0, ',', '.') }} transações
        </p>
    @endif
            ...
            <table class="w-full min-w-[960px] text-sm">
                <thead class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right">Terminal</th>
                        <th class="px-4 py-3 text-right">TPV</th>
                        <th class="px-4 py-3">Última venda</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    @forelse ($rows as $r)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60">
                            ...
                            <td class="px-4 py-3 text-right tabular-nums">{{ number_format((int) $r->qtd_terminal, 0, ',', '.') }}</td>
                            <td class="px-4 py-3 text-right tabular-nums font-semibold">R$ {{ number_format((float) $r->tpv, 2, ',', '.') }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $relatorio->data($r->ultima_venda) ?: '—' }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-10 text-center text-gray-500">Nenhum estabelecimento neste filtro.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
